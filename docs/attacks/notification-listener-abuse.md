...
**Android 4.3 (API 18)**: `NotificationListenerService` introduced. Any enabled listener receives all notifications.

**Android 5.0 (API 21)**: `cancelNotification(key)` added, replacing the tag/id-based cancellation. Malware can now precisely dismiss individual notifications.

**Android 8.0 (API 26)**: notification channels introduced. Notifications include channel information, giving listeners more context for filtering.

**Android 11 (API 30)**: notification history API (`getNotificationHistory()`). A listener can retroactively access recent notifications even if it was not running when they were posted.

**Android 13 (API 33)**: restricted settings for sideloaded apps. Apps installed via sideloading cannot direct users to notification access settings. Malware bypasses this by distributing through stores recognized by the installer, or by using session-based installers that set the correct installer package name.

**Android 14 (API 34)**: further tightening of restricted settings. The `RECEIVER_EXPORTED` flag requirement affects broadcast-based notification workarounds.
