...
**Android 10 (API 29)**: `android:exported` defaults to `false` for providers without intent filters.

**Android 12 (API 31)**: apps must explicitly set `android:exported` on all components.
...
Tools: [Drozer](https://github.com/WithSecureLabs/drozer) can enumerate and probe exported content providers automatically.
