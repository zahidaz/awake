...
**Android 5.0 (API 21)**: implicit intents to services throw `IllegalArgumentException`. Must use explicit intents.

**Android 12 (API 31)**: components with intent filters must explicitly declare `android:exported="true"` or `"false"`. Previously, having an intent filter implied export. This forces developers to make explicit decisions.

**Android 12+**: `PendingIntent` mutability must be declared (`FLAG_MUTABLE` / `FLAG_IMMUTABLE`), reducing PendingIntent hijacking.
