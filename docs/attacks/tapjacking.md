...
| Version | Mitigation |
|---------|-----------|
| Android 4.0.3 | `filterTouchesWhenObscured` attribute added. Activities can reject touches when an overlay is present. |
| Android 6.0 | `SYSTEM_ALERT_WINDOW` moved to special permission (Settings toggle required). |
| Android 8.0 | `TYPE_APPLICATION_OVERLAY` renders below system dialogs (permission prompts, install confirmations). |
| Android 12 | System dialogs set `FLAG_WINDOW_IS_PARTIALLY_OBSCURED`. Overlays become untouchable over sensitive system UI. |
...
`FLAG_NOT_TOUCHABLE` combined with a translucent or transparent overlay is the indicator. The overlay's dimensions and position relative to known dialog button locations are relevant for determining intent.
